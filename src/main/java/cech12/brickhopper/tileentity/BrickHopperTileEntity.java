...
public class BrickHopperTileEntity extends LockableLootTileEntity implements IHopper, ITickableTileEntity {

    private ItemStackHandler inventory = new ItemStackHandler(3);
    private int transferCooldown = -1;
    private long tickedGameTime;

    public BrickHopperTileEntity() {
        super(BrickHopperTileEntities.BRICK_HOPPER);
    }
    ...
    protected ITextComponent getDefaultName() {
        return new TranslationTextComponent("block.brickhopper.brick_hopper");
    }
    ...
    public static LazyOptional<Pair<IItemHandler, Object>> getItemHandler(World worldIn, double x, double y, double z, final Direction side) { //1.15
        ...
        if (block instanceof ISidedInventoryProvider) {
            return LazyOptional.of(() -> ImmutablePair.of(new SidedInvWrapper(((ISidedInventoryProvider)block).createInventory(state, worldIn, blockpos), side), state));
        }
