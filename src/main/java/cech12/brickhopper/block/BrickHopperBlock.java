...
public class BrickHopperBlock extends HopperBlock {
    ...
    public void addInformation(@Nonnull ItemStack stack, @Nullable IBlockReader worldIn, @Nonnull List<ITextComponent> tooltip, @Nonnull ITooltipFlag flagIn) {
        ...
        if (!ServerConfig.BRICK_HOPPER_PULL_ITEMS_FROM_WORLD_ENABLED.get()) {
            tooltip.add(new TranslationTextComponent("block.brickhopper.brick_hopper.desc.cannotAbsorbItemsFromWorld").mergeStyle(TextFormatting.RED));
        }
        if (!ServerConfig.BRICK_HOPPER_PULL_ITEMS_FROM_INVENTORIES_ENABLED.get()) {
            tooltip.add(new TranslationTextComponent("block.brickhopper.brick_hopper.desc.cannotAbsorbItemsFromInventories").mergeStyle(TextFormatting.RED));
        }
    }
